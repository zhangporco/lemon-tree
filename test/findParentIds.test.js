...
test('tree findParentIds 1', t => {
	const nodeId = 5;
	const res = new Lemon().findParentIds(tree, nodeId);
	t.is(res.length, 5);
});
test('tree findParentIds 2', t => {
	const nodeId = 2;
	const res = new Lemon().findParentIds(tree, nodeId);
	t.is(res[ 0 ], 1);
});
test('tree findParentIds 3', t => {
	const nodeId = 8;
	const res = new Lemon().findParentIds(tree, nodeId);
	t.is(res.length, 0);
});
...
test('tree children array 1', t => {
	...
	const res = tree.findParentIds(data, 3);
	t.is(res.length, 2);
});
...
test('tree children array 2', t => {
	...
	const tree = new Lemon({ id: 'menuId', children: 'children'});
	const res = tree.findParentIds(data, 411);
	t.is(res.length, 4);
});